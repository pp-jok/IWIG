...
`readiness` uses only `ready`, `partial`, `unavailable`, `failed`, `not_run`, `zero`, and `intentionally_omitted`. Stage completion itself remains in `processing`.
...
Evidence is factual: post fields, source artifacts, media, adaptive dense scans, representative frame timestamps, OCR, scene boundaries, text-change events, transcript spans, and evidence links. `scan_ref`, `frame_ref`, scene references and text-change references must resolve in the evidence registry; unresolved references are quality errors. `candidate_labels` are optional structural hints, not content analysis. IWIG does not use a visual-language model.

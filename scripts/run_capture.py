...
from content_package import build_timeline, extract_keyframes, field_status, find_existing_package, ocr_macos, select_structural_keyframes, should_reuse, srt, validate_content_package
...
def main() -> int:
    ...
    try:
        ...
        if result.get("transcript"):
            derived = output / "derived"; derived.mkdir(exist_ok=True)
            timeline = build_timeline(result["transcript"], (result.get("ocr") or {}).get("keyframes") or (result.get("keyframes") or {}).get("frames") or [])
            (derived / "timeline.json").write_text(json.dumps(timeline, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

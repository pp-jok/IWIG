...
def render_public_report(result: dict) -> str:
    ...
    lines += ["> 来源：faster-whisper small，本地 CPU int8 自动转写；可能存在听辨或断句错误。", "", *transcript] if transcript else ["[未生成：视频不可用、超出大小上限或本地转写失败。]"]
...
def main() -> int:
    ...
    try:
        ...
        video = output / result["media"]["video"]["path"]
        if video.stat().st_size <= args.max_video_mb * 1024 * 1024:
            try:
        ...
        (output / "capture.json").write_text(json.dumps(failed, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

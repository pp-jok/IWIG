...
def main() -> int:
    ...
    if args.run_dir and any(run.iterdir()):
        try:
            ...
            print("run_directory_identity_mismatch", file=sys.stderr); return 4
    forwarded = ["--url", args.url, "--run-dir", str(run), "--timeout", str(args.timeout), "--max-video-mb", str(args.max_video_mb)] + (["--force"] if args.force else []) + (["--keyframes"] if args.keyframes else []) + (["--ocr"] if args.ocr else []) + ([] if args.no_transcribe else ["--transcribe"])

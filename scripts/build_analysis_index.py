...
def build_analysis_index(run: Path, package: dict | None = None) -> dict:
    ...
    reference_errors = []
    for segment in derived.get("evidence_segments", []):
        for key, values in segment.items():
            if not key.endswith("_refs"):
                continue
            for reference in values or []:
                if reference not in evidence:
                    reference_errors.append(f"unresolved_evidence_ref:{segment.get('id')}:{key}:{reference}")
    if reference_errors:
        raise AnalysisIndexError("; ".join(reference_errors))
